...
from classeContador import VehicleCounter
...
if CAPTURE_FROM_VIDEO:
    IMAGE_SOURCE = "datasets/out-2_inv.avi" # Video file
else:
...
def process_frame(frame_number, frame, bg_subtractor, car_counter):
    ...
    fg_mask = bg_subtractor.apply(frame, None, 0.02)
    fg_mask = filter_mask(fg_mask)
    #cv2.imshow("Branco e preto", fg_mask)
    save_frame(IMAGE_DIR + "/imagens"+ "/mask_%04d.png"
...
def main():
    ...
    while True:
        ...
        if car_counter is None:
            ...
            car_counter = VehicleCounter(frame.shape[:2], frame.shape[0] * 2/3)
    ...
    print ("Veículos: %d" % car_counter.motocycle_count)
